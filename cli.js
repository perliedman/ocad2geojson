...
readOcad(filePath).then(ocadFile => {
  ...
  switch (mode) {
    case 'geojson':
      const options = {
        generateSymbolElements:
          argv['symbol-elements'] === undefined ||
          argv['symbol-elements'] !== 'false',
      }
    ...
    case 'qml':
      outStream.write(
        new XMLSerializer().serializeToString(ocadToQml(ocadFile))
      )
      break
    case 'params':
      ...
      break
    case 'symbols':
      const symbols = symNum
        ? ocadFile.symbols.filter(s => s.symNum === symNum)
        : ocadFile.symbols
